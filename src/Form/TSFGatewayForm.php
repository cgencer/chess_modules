...
namespace Drupal\chess_tsf\Form;
 ...
 */
class ChessTSFForm extends ConfigFormBase {
   ...
   */
	...
	public function buildForm(array $form, FormStateInterface $form_state) {
		$config = $this->configFactory->get('chess_tsf.settings');
		...
		$next_execution = $this->state->get('chess_tsf.next_execution');
		$next_execution = !empty($next_execution) ? $next_execution : REQUEST_TIME;

		$args = [
			'%time' => date_iso8601($this->state->get('chess_tsf.next_execution')),
			'%seconds' => $next_execution - REQUEST_TIME,
		];
		$form['status']['last'] = [
			'#type' => 'item',
			'#markup' => $this->t('chess_tsf_cron() will next execute the first time cron runs after %time (%seconds seconds from now)', $args),
		];
		...
		if ($this->currentUser->hasPermission('administer site configuration')) {
			...
			$form['cron_run']['cron_reset'] = [
				'#type' => 'checkbox',
				'#title' => $this->t("Run Chess-UKD's cron regardless of whether interval has expired."),
				'#default_value' => FALSE,
			];
		...
		$queue_1 = $this->queue->get('chess_tsf_queue_1');
		$queue_2 = $this->queue->get('chess_tsf_queue_2');
		...
		$form['cron_queue_setup']['queue'] = [
			...
			'#options' => [
				'chess_tsf_queue_1' => $this->t('Queue 1'),
				'chess_tsf_queue_2' => $this->t('Queue 2'),
			],
			'#default_value' => 'chess_tsf_queue_1',
		];
		...
		$form['configuration'] = [
			'#type' => 'details',
			'#title' => $this->t('Configuration of chess_tsf_cron()'),
			'#open' => TRUE,
		];
		$form['configuration']['chess_tsf_interval'] = [
			'#type' => 'select',
			'#title' => $this->t('Cron interval'),
			'#description' => $this->t('Time after which chess_tsf_cron will respond to a processing request.'),
	...
	public function cronRun(array &$form, FormStateInterface &$form_state) {
		$config = $this->configFactory->getEditable('chess_tsf.settings');

		$cron_reset = $form_state->getValue('cron_reset');
		if (!empty($cron_reset)) {
			$this->state->set('chess_tsf.next_execution', 0);
		}

		// Use a state variable to signal that cron was run manually from this form.
		$this->state->set('chess_tsf_show_status_message', TRUE);
		if ($this->cron->run()) {
	...
	public function addItems(array &$form, FormStateInterface &$form_state) {
		...
		// id attritbute.
		// @see \Drupal\chess_tsf\Plugin\QueueWorker\ReportWorkerOne
	...
	public function submitForm(array &$form, FormStateInterface $form_state) {
		...
		$this->configFactory->getEditable('chess_tsf.settings')
			->set('interval', $form_state->getValue('chess_tsf_interval'))
			->save();
